...
{
	/// <summary>
	/// A <see cref="RaTweenDynamic{TargetT, ValueT}"/> tween that is the base class that handles the logics of tweening the TextureOffset of a Material
	/// </summary>
	[Serializable]
	public abstract class RaTweenMaterialOffsetBase<TargetT> : RaTweenDynamic<TargetT, Vector2>
		where TargetT : UnityEngine.Object
	{
		...
		[Header("RaTweenMaterialOffset - Base")]
		...
		public RaTweenMaterialOffsetBase()
			: base()
		{

		}

		public RaTweenMaterialOffsetBase(TargetT target, Vector2 startValue, Vector2 endValue, float duration)
			: base(target, startValue, endValue, duration)
		{

		}

		public RaTweenMaterialOffsetBase(TargetT target, Vector2 endValue, float duration)
		...
		/// </summary>
		public RaTweenMaterialOffsetBase<TargetT> SetTargetPropertyDefault()
		{
			if(CanBeModified())
			{
		...
		public RaTweenMaterialOffsetBase<TargetT> SetTargetProperty(string propertyName)
		{
			if(CanBeModified())
			{
		...
		public RaTweenMaterialOffsetBase<TargetT> SetTargetProperty(int propertyID)
		{
			if(CanBeModified())
			{
		...
		public RaTweenMaterialOffsetBase<TargetT> SetExcludeAxis(Axis excludeAxes)
		{
			if(CanBeModified())
			{
		...
		public RaTweenMaterialOffsetBase<TargetT> OnlyIncludeAxis(Axis inclAxes)
		{
			if(CanBeModified())
			{
		...
		{
			RaTweenMaterialOffsetBase<TargetT> tween = MaterialOffsetClone();
		...
		protected override Vector2 ReadValue(TargetT reference)
		{
			...
			switch(_propertyType)
			{
				...
				case RaPropertyDefaultType.PropertyName:
					return material.GetTextureOffset(_propertyName);
				case RaPropertyDefaultType.PropertyID:
					return material.GetTextureOffset(_propertyID);
			}
		...
		protected abstract RaTweenMaterialOffsetBase<TargetT> MaterialOffsetClone();
		...
		private void WriteValue(TargetT reference, Vector2 value)
		{
			...
			switch(_propertyType)
			{
				case RaPropertyDefaultType.Default:
					...
					break;
				case RaPropertyDefaultType.PropertyName:
					material.SetTextureOffset(_propertyName, value);
					break;
				case RaPropertyDefaultType.PropertyID:
					material.SetTextureOffset(_propertyID, value);
					break;
				default:
